    ...
    useEffect(() => {
        const fetchProductsWithCategories = async () => {
            const productWithCategory =
                await medusa.client.admin.products.retrieve(product.id, {
    ...
    return (
        <>
            <Container title="Categories Images">
                ...
                {!loading && (
                    <div>
                        Categories Images. Found{" "}
                        {medusaProduct?.categories?.length} categories
                    </div>
                )}

                {!loading &&
                    medusaProduct.categories.map((category) => (
                        <div key={category.id} className="mt-3 w-full">
                            <div className="flex items-center">
                                <div className="inter-base-semibold flex-1">
                                    {category.name}
                                </div>
                                <DropdownMenu>
